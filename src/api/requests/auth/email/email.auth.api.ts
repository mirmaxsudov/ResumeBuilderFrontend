...
const verifyEmail = async (email: string, code: string): Promise<ApiResponse<string>> => {
    const response = await $api.post<Promise<ApiResponse<string>>>(BASE_EMAIL_URL + "/verify-email", null, {
        params: {
            email,
            code
        }
